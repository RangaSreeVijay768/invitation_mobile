...
import {NgHttpCachingEntry} from "@core/rg-http-caching/ng-http-caching-service.service";
...
export function serializeRequest(req: HttpRequest<any>): string {
    ...
    return JSON.stringify({
        headers:  // Just a helper to make this into an object, not really required but makes the output nicer
        ...
        params:  // Just a helper to make this into an object, not really required but makes the output nicer
            request.headers.keys().map( // Get all of the headers
                (key: string) => [key, request.headers.getAll(key)] // Get all of the corresponding values for the headers
